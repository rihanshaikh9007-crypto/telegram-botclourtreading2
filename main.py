...
bot_main = telebot.TeleBot(MAIN_TOKEN)
bot_finance = telebot.TeleBot(DEPOSIT_TOKEN)
...
def wingo_menu(call):
    ...
    markup.row(
        InlineKeyboardButton("GREEN", callback_data=f"bet_grn_{mode}", **{'style': 'success'}), 
        InlineKeyboardButton("RED", callback_data=f"bet_red_{mode}", **{'style': 'danger'}), 
        InlineKeyboardButton("VIOLET", callback_data=f"bet_vio_{mode}", **{'style': 'primary'})
    )
    markup.row(
        InlineKeyboardButton("BIG", callback_data=f"bet_big_{mode}", **{'style': 'primary'}), 
        InlineKeyboardButton("SMALL", callback_data=f"bet_sml_{mode}", **{'style': 'primary'})
    )
    markup.add(InlineKeyboardButton("🔙 Back", callback_data="back_main", **{'style': 'danger'}))
    
    text = f"🎮 *WINGO {mode} MIN*\n🆔 *Period:* `{period_id}`\n⏳ *Time Remaining:* `{time_left} Seconds`\n💰 *Balance:* `₹{balance}`\n\n👇 Apna Prediction chunein:"
    try: bot_main.edit_message_text(text, call.message.chat.id, call.message.message_id, reply_markup=markup, parse_mode="Markdown")
    except: pass
...
if __name__ == "__main__":
    ...
    threading.Thread(target=lambda: bot_main.infinity_polling(allowed_updates=['message', 'callback_query', 'chat_join_request'])).start()
    threading.Thread(target=lambda: bot_finance.infinity_polling()).start()
